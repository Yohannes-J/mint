...
const ResultFrameworkMenu = ({ open = true }) => {
  ...
  // Create base path from role, replace spaces with hyphens
  ...
  <li key={sector._id} className="rounded overflow-hidden">
    <div
      ...
      // Remove toggle on whole div click, only arrow will toggle
      // onClick={() => sector.subsectors && toggleSector(sector._id)}
      // role and tabIndex removed because no longer clickable div
    >
      <Link
        ...
        // Remove onClick preventing navigation
        // onClick={e => sector.subsectors && sector.subsectors.length && e.preventDefault()}
      >
        {sector.name}
      </Link>
      ...
      {sector.subsectors && sector.subsectors.length > 0 && (
        <button
          ...
          aria-expanded={!!openSectors[sector._id]}
          aria-label={`Toggle subsectors for ${sector.name}`}
          type="button"
        >
          <ChevronDown
            size={16}
